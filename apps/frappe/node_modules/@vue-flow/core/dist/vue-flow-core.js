...
const NodeWrapper = vue.defineComponent({
  ...
  setup(props) {
    ...
    const parentNode = vue.computed(() => findNode(node.value.parentNode));
    const connectedEdges = vue.computed(() => getConnectedEdges([node.value], edges.value));
...
function useActions(id2, emits, hooksOn, state, getters, nodeIds, edgeIds) {
  ...
  const fromObject = (obj) => {
    ...
    if ((viewport == null ? void 0 : viewport.x) && (viewport == null ? void 0 : viewport.y) || position) {
      ...
      until(() => viewportHelper.value.initialized).toBe(true).then(() => {
  ...
  until(() => viewportHelper.value.initialized).toBe(true, { flush: "pre" }).then(() => {
